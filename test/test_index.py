...
def test_insert_multiple(git_gecko):
    ...
    assert idx.get(("key1", "key2")) == {"some_example_data", "more_example_data"}
    assert idx.get(("key1",)) == {"some_example_data", "more_example_data"}
    idx.save()
    assert idx.get(("key1", "key2")) == {"some_example_data", "more_example_data"}
    assert idx.get(("key1",)) == {"some_example_data", "more_example_data"}
...
def test_build(git_gecko):
    ...
    idx.build([(("key1", "key2"), "some_example_data"), (("key1", "key4"), "new_data")], [])
    assert idx.get(("key1", "key2")) == {"some_example_data"}
    assert idx.get(("key1", "key3")) == set()
