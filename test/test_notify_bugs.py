...
def test_fallback_test_ids_to_paths(env):
    ...
    assert bugs.fallback_test_ids_to_paths(test_ids) == {
        ...
        "testing/web-platform/mozilla/tests/tests/example.html": ["/_mozilla/tests/example.html"],
    }
...
def test_fx_only(env):
    ...
    with patch(
        ...
        with patch(
            "sync.notify.bugs.test_ids_to_paths",
            return_value={"testing/web-platform/tests/test/test.html": ["/test/test.html"]},
        ):
            bug_data = bugs.for_sync(sync, results_obj)

    assert list(bug_data.values()) == [
        [("/test/test.html", None, results_obj.test_results["/test/test.html"], None)]
    ]
...
def test_crash(env):
    ...
    assert list(bug_data.values()) == [
        [("/test/test.html", None, results_obj.test_results["/test/test.html"], "CRASH")]
    ]
...
def test_update_metadata(env, git_gecko, git_wpt, pull_request, git_wpt_metadata, mock_mach):
    ...
    mock_mach.set_data(
        "wpt-test-paths", json.dumps(bugs.fallback_test_ids_to_paths(["/test/test.html"]))
    )
...
def test_already_linked(env):
    ...
    results_obj.test_results["/test/test.html"].bug_links.append(
        MetaLink(
            None, "/test/test.html", f"{env.bz.bz_url}/show_bug.cgi?id=10000", "firefox", None, None
        )
    )
    sync = Mock()
    # The Mock class defines a _lock property which interferes with our one;
    ...
    assert (
        "Creating a bug in component Testing :: web-platform-tests" not in env.bz.output.getvalue()
    )
...
def test_make_summary():
    ...
    assert (
        bugs.make_summary(test_results, prefix)
        == "Test failures in /a/b/ [c.html, d.html, e.html, and 2 others]"
    )
    assert (
        bugs.make_summary(test_results, prefix, max_length=55)
        == "Test failures in /a/b/ [c.html, d.html, and 3 others]"
    )
    assert (
        bugs.make_summary(test_results, prefix, max_length=50)
        == "Test failures in /a/b/ [c.html, and 4 others]"
    )
    assert (
        bugs.make_summary(test_results, prefix, max_length=35) == "Test failures in /a/b/ [5 tests]"
    )
    assert bugs.make_summary(test_results, prefix, max_length=25) == "Test failures in /a/b/"
    assert bugs.make_summary(test_results, prefix, max_length=15) == "Test failures"

    test_results = ["/a/b/c.html", "/a/b/d.html", "/a/b/e.html"]
    test_results = [(item, None, None) for item in test_results]
    assert (
        bugs.make_summary(test_results, prefix) == "Test failures in /a/b/ [c.html, d.html, e.html]"
    )
