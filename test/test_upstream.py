...
def test_create_pr_backout(env, git_gecko, git_wpt, upstream_gecko_commit, upstream_gecko_backout):
    bug = 1234
    test_changes = {"README": b"Change README\n"}
    rev = upstream_gecko_commit(test_changes=test_changes, bug=bug, message=b"Change README")
...
def test_create_pr_revert(env, git_gecko, git_wpt, upstream_gecko_commit, upstream_gecko_revert):
    bug = 1234
    test_changes = {"README": b"Change README\n"}
...
def test_create_pr_backout_reland(
    env, git_gecko, git_wpt, upstream_gecko_commit, upstream_gecko_backout
):
    bug = 1234
    test_changes = {"README": b"Change README\n"}
    rev = upstream_gecko_commit(test_changes=test_changes, bug=bug, message=b"Change README")
    ...
    upstream_gecko_commit(other_changes=test_changes, bug=1235, message=b"Change other file")

    relanding_rev = upstream_gecko_commit(
        test_changes=test_changes, bug=bug, message=b"Reland: Change README"
    )

    update_repositories(git_gecko, git_wpt, wait_gecko_commit=rev)
    env.lando.hg_to_git[relanding_rev] = "test_revision"
...
def test_create_partial_backout_reland(
    env, git_gecko, git_wpt, upstream_gecko_commit, upstream_gecko_backout
):
    bug = 1234
    test_changes = {"README": b"Change README\n"}
    rev0 = upstream_gecko_commit(test_changes=test_changes, bug=bug, message=b"Change README")
    rev1 = upstream_gecko_commit(
        test_changes={"README": b"Change README again\n"}, bug=bug, message=b"Change README again"
    )

    update_repositories(git_gecko, git_wpt)
    env.lando.hg_to_git[rev1] = "test_revision"
    ...
    relanding_rev = upstream_gecko_commit(
        test_changes={"README": b"Change README once more\n"},
        bug=bug,
...
def test_land_pr_after_status_change(
    env, git_gecko, git_wpt, hg_gecko_upstream, upstream_gecko_commit, set_pr_status
):
    bug = 1234
    test_changes = {"README": b"Change README\n"}
    rev = upstream_gecko_commit(test_changes=test_changes, bug=bug, message=b"Change README")
...
def test_no_upstream_downstream(
    env, git_gecko, git_wpt, upstream_gecko_commit, upstream_gecko_backout
):
    hg_rev = upstream_gecko_commit(
        test_changes={"README": b"Example change"},
...
wpt-commits: 0000000000000000000000000000000000000000""",
    ...
    env.lando.hg_to_git[hg_rev] = "test_revision"
    pushed, landed, failed = upstream.gecko_push(
...
def test_upstream_existing(env, git_gecko, git_wpt, upstream_gecko_commit, upstream_wpt_commit):
    ...
    upstream_gecko_commit(test_changes=test_changes_1, bug=bug, message=b"Change README")
    test_changes_2 = {"OTHER": b"Add other file\n"}
    gecko_rev_2 = upstream_gecko_commit(test_changes=test_changes_2, bug=bug, message=b"Add other")

    upstream_wpt_commit(file_data=test_changes_1)
    update_repositories(git_gecko, git_wpt, wait_gecko_commit=gecko_rev_2)
    git_rev = "test_revision"
    env.lando.hg_to_git[gecko_rev_2] = git_rev
    pushed, landed, failed = upstream.gecko_push(
    ...
    env.lando.hg_to_git[gecko_rev_3] = git_rev
    pushed, landed, failed = upstream.gecko_push(
...
def test_upstream_multi(env, git_gecko, git_wpt, upstream_gecko_commit):
    ...
    git_rev = "test_revision"
    env.lando.hg_to_git[rev_0] = git_rev
    pushed, _, _ = upstream.gecko_push(git_gecko, git_wpt, "autoland", rev_0, raise_on_error=True)
    assert len(pushed) == 1
    sync_0 = pushed.pop()
    ...
    env.lando.hg_to_git[rev_1] = git_rev
    pushed, _, _ = upstream.gecko_push(git_gecko, git_wpt, "autoland", rev_1, raise_on_error=True)
    assert len(pushed) == 1
    ...
    env.lando.hg_to_git[rev_2] = git_rev
    pushed, _, _ = upstream.gecko_push(git_gecko, git_wpt, "autoland", rev_2, raise_on_error=True)
    ...
    env.lando.hg_to_git[rev_3] = git_rev
    pushed, landed, failed = upstream.gecko_push(
...
def test_upstream_reprocess_commits(
    env, git_gecko, git_wpt, upstream_gecko_commit, upstream_gecko_backout
):
    bug = 1234
    test_changes = {"README": b"Change README\n"}
    ...
    sync_point.commit = sync_commit.GeckoCommit(
...
def setup_repo(env, git_wpt, git_gecko, hg_gecko_upstream, upstream_gecko_commit, set_pr_status):
    bug = 1234
    changes = {"README": b"Changes to README\n"}
    upstream_gecko_commit(test_changes=changes, bug=bug, message=b"Change README")
...
def test_pr_commits_merge(
    ...
    sync = setup_repo(
        env, git_wpt, git_gecko, hg_gecko_upstream, upstream_gecko_commit, set_pr_status
    )
...
def test_pr_commits_squash_merge(
    env,
    git_wpt,
    git_gecko,
    git_wpt_upstream,
