...
from typing import Any, IO, Iterable, Iterator, Optional, Sequence, Union, cast, TYPE_CHECKING
...
class LandingSync(SyncProcess):
    ...
    def __init__(self, git_gecko: Repo, git_wpt: Repo, process_name: ProcessName) -> None:
        ...
        self._unlanded_gecko_commits: Optional[list[sync_commit.GeckoCommit]] = None
    ...
    def new(
        ...
        if bug is None:
            bug = env.bz.new(
                "Update web-platform-tests to %s" % wpt_head,
                "",
                "Testing",
                "web-platform-tests",
    ...
    def has_metadata(cls, message: bytes) -> bool:
        required_keys = ["wpt-head", "wpt-type"]
        metadata = sync_commit.get_metadata(message)
        return (
            all(item in metadata for item in required_keys)
            and metadata.get("wpt-type") == "landing"
        )

    def unlanded_gecko_commits(self) -> Sequence[sync_commit.GeckoCommit]:
        ...
        if self._unlanded_gecko_commits is None:
            ...
            def on_integration_branch(commit: sync_commit.Commit) -> bool:
                ...
                return self.git_gecko.is_ancestor(
                    commit.commit, self.git_gecko.rev_parse(self.gecko_integration_branch())
                )
            ...
            for status in ["open", "wpt-merged"]:
                unlanded_syncs |= set(
                    upstream.UpstreamSync.load_by_status(self.git_gecko, self.git_wpt, status)
                )

            for sync in unlanded_syncs:
                branch_commits = [
                    commit.sha1 for commit in sync.gecko_commits if on_integration_branch(commit)
                ]
                if branch_commits:
                    logger.info(
                        "Commits from unlanded sync for bug %s (PR %s) will be reapplied"
                        % (sync.bug, sync.pr)
                    )
            ...
            unlanded_commits = self.git_wpt.iter_commits(
                "%s..origin/master" % self.wpt_commits.base.sha1
            )
            seen_bugs = set()
            for commit in unlanded_commits:
                wpt_commit = sync_commit.WptCommit(self.git_wpt, commit)
                gecko_commit_sha = wpt_commit.metadata.get("gecko-commit")
                if gecko_commit_sha:
                    git_sha = cinnabar(self.git_gecko).hg2git(gecko_commit_sha)
                    gecko_commit = sync_commit.GeckoCommit(self.git_gecko, git_sha)
                    bugzilla_url = gecko_commit.metadata.get("bugzilla-url")
                    bug_number = bug.bug_number_from_url(bugzilla_url)
                    if on_integration_branch(gecko_commit):
                        if bug_number and bug_number not in seen_bugs:
                            logger.info(
                                "Commits from landed sync for bug %s will be reapplied" % bug_number
                            )
                            seen_bugs.add(bug_number)
                        commits.append(gecko_commit.sha1)

            commit_shas = set(commits)

            # Order the commits according to the order in which they landed in gecko
            ordered_commits = []
            for commit in self.git_gecko.iter_commits(
                self.gecko_integration_branch(), paths=env.config["gecko"]["path"]["wpt"]
            ):
                if commit.hexsha in commit_shas:
                    ordered_commits.append(commit.hexsha)
                    commit_shas.remove(commit.hexsha)
                if not commits:
                    break

            self._unlanded_gecko_commits = list(
                reversed(
    ...
    def has_metadata_for_sync(self, sync: DownstreamSync) -> bool:
        for item in reversed(list(self.gecko_commits)):
            if (
                item.metadata.get("wpt-pr") == sync.pr
                and item.metadata.get("wpt-type") == "metadata"
            ):
                return True
        return False
    ...
    def landing_commit(self) -> Any | None:
        ...
        if (
            head.metadata.get("wpt-type") == "landing"
            and head.metadata.get("wpt-head") == self.wpt_commits.head.sha1
        ):
            return head
        return None
    ...
    def add_pr(
        ...
        metadata = {"wpt-pr": pr_id, "wpt-commits": ", ".join(item.sha1 for item in wpt_commits)}
...
"""
        ...
        if copy:
            commit = self.copy_pr(
                git_work_gecko, git_work_wpt, pr, wpt_commits, message, author, metadata
            )
        else:
            commit = self.move_pr(
                git_work_gecko,
    ...
    def copy_pr(
        ...
        git_work_wpt: Repo,
        pr: AttrDict,
        wpt_commits: Sequence[sync_commit.WptCommit],
        message: bytes,
        author: str,
        ...
        dest_path = os.path.join(git_work_gecko.working_dir, env.config["gecko"]["path"]["wpt"])
        ...
        git_work_wpt.head.reset(wpt_commits[-1].commit, index=True, working_tree=True)
        ...
        def ignore_names(src: str, names: Iterable[str]) -> list[str]:
            rv = []
            for item in names:
        ...
        shutil.copytree(src_path, dest_path, ignore=ignore_names)

        # Now re-checkout the files we don't want to change
        # checkout-index allows us to ignore files that don't exist
        git_work_gecko.git.checkout_index(
            *(os.path.join(env.config["gecko"]["path"]["wpt"], item) for item in keep_paths),
            force=True,
            quiet=True,
        )
        ...
        git_work_gecko.git.add(env.config["gecko"]["path"]["wpt"], no_ignore_removal=True)

        gecko_commit = sync_commit.GeckoCommit.create(
            self.git_gecko, message, metadata, author=author.encode("utf8"), allow_empty=allow_empty
        )
    ...
    def move_pr(
        ...
        return sync_commit.move_commits(
            self.git_wpt,
            revish,
            message,
            git_work_gecko,
    ...
    def reapply_local_commits(self, gecko_commits_landed: set[str]) -> None:
        ...
        try:
            ...
            err_msg = f"Landing wpt failed because reapplying commits failed:\n{e.message}"
    ...
    def add_metadata(self, sync: DownstreamSync) -> None:
        ...
        if not success:
            ...
            affected_metadata = {
                os.path.join(env.config["gecko"]["path"]["meta"], item) + ".ini"
        ...
        if metadata_commit.msg.startswith(b"Bug None"):
            ...
            new_message = b"Bug %s%s" % (
                str(bug_number).encode("utf8"),
    ...
    def apply_prs(self, prev_wpt_head: str, landable_commits: LandableCommits) -> None:
        ...
        def update_gecko_landed(
            sync: DownstreamSync | UpstreamSync, commits: list[WptCommit]
        ) -> None:
            if isinstance(sync, upstream.UpstreamSync):
            ...
            if last_applied_seen:
                ...
                try:
                    ...
                    if isinstance(sync, downstream.DownstreamSync):
                        ...
                        raise AbortError(
                            "Expected an existing gecko commit for PR %s, but not found" % (pr,)
                        )
        ...
        if have_prs:
            raise AbortError(
                "Found unexpected gecko commit for PRs %s"
        ...
        if pr_count_applied + pr_count_upstream_empty + pr_count_unapplied != len(landable_commits):
            raise AbortError(
                "PR counts don't match; got %d applied, %d unapplied %d upstream"
        ...
        for i, (pr, sync, commits, meta_only) in unapplied_commits:
            ...
            if not meta_only:
                ...
                pr_commit = self.add_pr(pr, sync, commits, prev_wpt_head=prev_wpt_head, copy=copy)
            prev_wpt_head = commits[-1].sha1
            if pr_commit:
                if copy:
    ...
    def update_landing_commit(self) -> GeckoCommit:
        ...
        if not self.landing_commit:
            metadata = {"wpt-type": "landing", "wpt-head": self.wpt_commits.head.sha1}
...
MANUAL PUSH: wpt sync bot
            ...
            sync_commit.create_commit(
                git_work, self.landing_commit.msg, allow_empty=True, amend=True, no_edit=True
            )
    ...
    def update_sync_point(self, sync_point: SyncPoint) -> None:
        ...
        with open(
            os.path.join(
                gecko_work.working_dir, env.config["gecko"]["path"]["meta"], "mozilla-sync"
            ),
            "wb",
        ) as f:
            sync_point.dump(f)
        if gecko_work.is_dirty():
            gecko_work.index.add(
                [os.path.join(env.config["gecko"]["path"]["meta"], "mozilla-sync")]
            )
            self.update_landing_commit()
    ...
    def next_try_push(self, retry: bool = False) -> TryPush | None:
        ...
        if retry:
            ...
            stability = latest_try_push is not None and not latest_try_push.infra_fail
    ...
    def try_result(
        self, try_push: TryPush | None = None, tasks: TryPushTasks | None = None
    ) -> TryPushResult:
        ...
        if tasks.failed_builds() and not try_push.accept_failures:
            return TryPushResult.infra_fail
        if tasks.failure_limit_exceeded(target_success_rate) and not try_push.accept_failures:
...
def push_with_lando(landing: LandingSync) -> bytes:
...
def unlanded_with_type(
    git_gecko: Repo, git_wpt: Repo, wpt_head: Optional[str], prev_wpt_head: str
) -> Iterator[tuple[Optional[int], list[WptCommit], LandableStatus]]:
    pr_commits = unlanded_wpt_commits_by_pr(
        git_gecko, git_wpt, wpt_head or prev_wpt_head, "origin/master"
    )
...
def unlanded_wpt_commits_by_pr(
    ...
    for commit in git_wpt.iter_commits(revish, reverse=True, first_parent=True):
        ...
        if pr is None or pr == 0 or pr not in index_by_pr:
            ...
            if len(commit.parents) > 1:
                ...
                for merged_commit in git_wpt.iter_commits(merged_revish, reverse=True):
            ...
            assert pr_data[0] == pr
            index_by_pr = {
                key: (value if value < idx else value - 1) for key, value in index_by_pr.items()
            }
...
def landable_commits(
    ...
    for pr, commits in pr_commits:
        ...
        def upstream_sync(bug_number: int) -> Optional[UpstreamSync]:
            syncs = upstream.UpstreamSync.for_bug(
                git_gecko, git_wpt, bug_number, statuses=None, flat=True
            )
            for sync in syncs:
                if sync.merge_sha == commits[-1].sha1 and not sync.wpt_commits:
                    ...
                    with SyncLock("upstream", None) as lock:
                        ...
                        with sync.as_mut(lock):
                            ...
                            # here will be empty
                            sync.set_wpt_base(
                                sync_commit.WptCommit(git_wpt, commits[0].sha1 + "~").sha1
                            )
        ...
        if not include_incomplete:
            if not sync:
                ...
                last = True
            elif isinstance(sync, downstream.DownstreamSync) and sync.landable_status not in (
...
@entry_point("landing")
def wpt_push(
    git_gecko: Repo, git_wpt: Repo, commits: list[str], create_missing: bool = True
) -> None:
    prs = set()
...
def update_landing(
    ...
    with SyncLock("landing", None) as lock:
        ...
        if landing is None:
            ...
            blocks = [
                sync.bug
                for (pr_, sync, commits_) in commits
                if isinstance(sync, downstream.DownstreamSync) and sync.bug is not None
            ]
            assert landing.bug is not None
            ...
            if prev_wpt_head and landing.wpt_commits.base.sha1 != prev_wpt_head:
                raise AbortError(
                    "Existing landing base commit %s doesn't match"
                    "supplied previous wpt head %s" % (landing.wpt_commits.base.sha1, prev_wpt_head)
                )
        ...
        with landing.as_mut(lock):
            if landing.latest_try_push is None:
                ...
                if try_push.status == "complete" and (try_result.is_ok() or accept_failures):
                    try:
                        landing.gecko_rebase(landing.gecko_landing_branch())
                    except AbortError:
                        message = record_rebase_failure(landing)
                    ...
                    logger.info(
                        "Existing try push %s is waiting for try results" % try_push.treeherder_url
                    )
                else:
                    logger.info(
                        "Existing try push %s requires manual fixup" % try_push.treeherder_url
                    )
...
def needinfo_users() -> list[str]:
    needinfo_users = [
        item.strip() for item in (env.config["gecko"]["needinfo"].get("landing", "").split(","))
    ]
...
def record_build_failures(sync: LandingSync, try_push: TryPush) -> str:
...
def record_infra_fail(sync: LandingSync, try_push: TryPush) -> str:
...
def record_rebase_failure(sync: LandingSync) -> str:
...
def push_to_gecko(
    git_gecko: Repo, git_wpt: Repo, sync: LandingSync, allow_push: bool = True
) -> None:
    if not allow_push:
        logger.info(
            "Landing in bug %s is ready for push.\n"
            "Working copy is in %s" % (sync.bug, sync.gecko_worktree.get().working_dir)
        )
        return
                        ...
                        try:
                            ...
                            if landing_is_complete:
                                ...
                                if sync.bug is not None and not sync.results_notified:
                                    env.bz.comment(
                                        sync.bug, "Test result changes from PR not available."
                                    )
...
def gecko_push(
    ...
    last_sync_point, base_commit = LandingSync.prev_gecko_commit(git_gecko, repository_name)
    ...
    for commit in git_gecko.iter_commits(revish, reverse=True):
        ...
        if landed_central and gecko_commit.is_landing:
            ...
            if gecko_commit.bug is None:
                logger.error("Commit %s looked link a landing, but had no bug" % gecko_commit.sha1)
                continue
            syncs = LandingSync.for_bug(
                git_gecko, git_wpt, gecko_commit.bug, statuses=None, flat=True
            )
            if syncs:
                sync = syncs[0]
            ...
            for backed_out_commit in backed_out:
                ...
                syncs = []
                if bug is not None:
                    syncs = LandingSync.for_bug(git_gecko, git_wpt, bug, statuses=None, flat=True)
                if syncs:
