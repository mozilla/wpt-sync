...
@mut('sync')
def for_sync(sync: DownstreamSync, results: Results) -> Mapping[int, List[ResultsEntryStatus]]:
...
def get_common_prefix(test_ids: Iterable[Text]) -> Tuple[List[Tuple[Text, ...]], Tuple[Text, ...]]:
    ...
    common_prefix: ignore = os.path.commonprefix([item[:-1] for item in split_names])
