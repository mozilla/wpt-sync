...
from ..lock import mut, MutGuard, SyncLock
...
class TriageBugs(object):
    ...
    def meta_links(self):
        ...
        for link in self.wpt_metadata.iterbugs(test_id=None,
            ...
            bug = env.bz.id_from_url(link.url, bugzilla_url)
            rv[bug].append(link)
        return rv
...
def update_triage_bugs(git_gecko):
    ...
    for bug in triage_bugs.updated_bugs(meta_links.keys()):
        if bug.resolution == "INVALID":
            ...
            updates.append[bug.id] = bug._bug["dupe_of"]
    ...
    with SyncLock(TriageBugs.process_name) as lock:
    ...
    for bug, old_links in iteritems(removed_by_bug):
        comment_removed(bug, old_links)


def comment_removed(bug_id, links):
    ...
    with env.bx.bug_ctx(bug_id) as bug:
        bug.add_comment("""Resolving bug as invalid removed the following wpt triage data:
